Shader "UI/HoverGlowSweep"
{
    ...
    Properties
    {
        ...
        _StartTime ("Start Time (per-button reset hook)", Float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float active = step(phase, _SweepDuration);
                float u = saturate(phase / _SweepDuration); // 0..1
                // 2페이즈: P1(u<0.6=0.3s) 위치0->1/3·두께0.5->4.0·투명0.2->0.8 / P2(u>=0.6=0.2s) 위치1/3->1·두께4.0->1.0·투명0.8->0.4
                float pos, opacity, thickMul;
                if (u < 0.6) { float f = u / 0.6;         pos = f * (1.0/3.0);             thickMul = lerp(0.5, 4.0, f); opacity = lerp(0.2, 0.8, f); }
                else         { float f = (u - 0.6) / 0.4; pos = (1.0/3.0) + f * (2.0/3.0); thickMul = lerp(4.0, 1.0, f); opacity = lerp(0.8, 0.4, f); }
                ...
                float band = 1.0 - saturate(abs(s - center) / bw);
                band *= band;
