...
public class LevelZoneLayoutLoader : MonoBehaviour
{
    ...
    private void LoadZoneLevelData(LevelZoneSlot zone, LevelData levelData)
    {
        ...
        SpawnTutorialObjects(levelData, offset);
    ...
    private void SpawnDecorativeObjects(LevelData levelData, Vector2Int offset)
    {
        if (prefabRegistry == null)
            return;
        ...
        for (int i = 0; i < placements.Count; i++)
        {
            ...
            {
                Debug.LogWarning(
                    $"LevelZoneLayoutLoader could not find a decorative object prefab for key '{placement.PrefabKey}'.",
                    this);
                continue;
            }

            SpawnDecorativeObjectGameObject(prefab, placement.GridPosition + offset, parent);
        }
    }

    private void SpawnTutorialObjects(LevelData levelData, Vector2Int offset)
    {
        if (prefabRegistry == null)
            return;

        IReadOnlyList<TutorialObjectPlacementData> placements = levelData.TutorialObjectPlacements;
        ...
        for (int i = 0; i < placements.Count; i++)
        {
            ...
            {
                Debug.LogWarning(
                    $"LevelZoneLayoutLoader could not find a tutorial object prefab for key '{placement.PrefabKey}'.",
                    this);
                continue;
            }

            SpawnTutorialObjectGameObject(prefab, placement.GridPosition + offset, parent);
        }
    }
