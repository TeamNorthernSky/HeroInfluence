...
namespace JC.VFX
{
    ...
    public static class JusticeTrailPresetRuntime
    {
        ...
        /// <summary>
        /// 수명 그라데이션 공통 조립.
        /// 본체는 팔레트 × bodyEmission, 머리 구간에는 별도 배수를 가진 하이라이트를 얹는다.
        /// 하이라이트를 재질 색에 섞지 않는 이유: 재질에 섞으면 전체가 탈색되어 채도가 무너진다.
        /// </summary>
        static Gradient BuildCore(JusticeTrailPreset p, Color hlColor, float hlRatio, float hlEmission,
                                  float bodyEmission, float chromaHold, float alphaHold)
        {
            float hold = Mathf.Clamp(alphaHold, 0f, 0.95f);
            Color head = Boost(p.strokeHeadColor, bodyEmission, chromaHold);
            Color mid = Boost(p.strokeMidColor, bodyEmission, chromaHold);
            Color tail = Boost(p.strokeTailColor, bodyEmission, chromaHold);
        ...
        static Gradient BuildTrailGradient(JusticeTrailPreset p, JusticeTrailPreset.TrailGroup g) =>
            BuildCore(p, g.highlightColor, g.highlightRatio, g.highlightEmission, g.emission, g.chromaHold, g.alphaHold);

        /// <summary>타격 스파크용 그라데이션. 본체는 팔레트, 머리 구간은 타격 하이라이트.</summary>
        public static Gradient BuildImpactGradient(JusticeTrailPreset p) =>
            BuildCore(p, p.impact.highlightColor, p.impact.highlightRatio, p.impact.highlightEmission, 1f, 1f, 0.5f);
        ...
        static void ApplyCommon(ParticleSystem ps, JusticeTrailPreset.EmitGroupBase g)
        {
            ...
            main.gravityModifier = g.gravity;
        ...
        public static void ApplyTrailGroup(ParticleSystem ps, JusticeTrailPreset.TrailGroup g, JusticeTrailPreset p)
        {
            if (ps == null || g == null || p == null) return;
            ApplyCommon(ps, g);
            ...
            col.enabled = true;
            col.color = new ParticleSystem.MinMaxGradient(BuildTrailGradient(p, g));
        ...
        /// </summary>
        public static void ApplySparkGroup(ParticleSystem ps, JusticeTrailPreset.SparkGroup g, JusticeTrailPreset p,
        ...
        public static void ApplyImpact(GameObject root, JusticeTrailPreset p, Material flashMaterial = null)
        {
            ...
            if (ps != null)
            {
                ...
                main.startSize = new ParticleSystem.MinMaxCurve(im.sizeMin, Mathf.Max(im.sizeMin, im.sizeMax));
                main.gravityModifier = im.gravity;
                ...
                var rend = ps.GetComponent<ParticleSystemRenderer>();
                if (rend != null) rend.lengthScale = im.lengthScale;
            }
        ...
        public static void ApplyTrail(GameObject root, JusticeTrailPreset p, Material sparkMaterial = null)
        {
            ...
            {
                if (ps.gameObject.name == TrailName) ApplyTrailGroup(ps, p.trail, p);
                else if (ps.gameObject.name == SparkName) ApplySparkGroup(ps, p.spark, p, sparkMaterial);
            }
    ...
    public class JusticeTrailPresetBinder : MonoBehaviour
    {
        ...
        [Tooltip("읽어올 프리셋. 비우면 아무것도 하지 않는다.")]
        ...
        public void ApplyNow()
        {
            ...
            if (kind == Kind.Trail) JusticeTrailPresetRuntime.ApplyTrail(gameObject, preset, sparkMaterial);
            else JusticeTrailPresetRuntime.ApplyImpact(gameObject, preset, flashMaterial);

            if (!applyMaterials) return;

            // 자기 종류의 재질만 만진다.
            // 예전엔 타격 프리팹도 strokeMaterial을 들고 있어, 스폰할 때마다 궤적 재질을 덮어썼다.
            if (kind == Kind.Trail) JusticeTrailPresetRuntime.ApplyMaterials(preset, strokeMaterial, null);
            else JusticeTrailPresetRuntime.ApplyMaterials(preset, null, impactMaterial);
        }
    }
}
